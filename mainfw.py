...
while True:  
    ...
    if inputbox[0] == "0":
        ...
        print("\033[1m"+"\033[37m"+"(매수) *실패* 보유 금액이 부족합니다.")
    else:
        haveconin += int(inputbox[0])
        usermoney -= int(inputbox[0]) * int(coinmoney)
        print("\033[1m"+"\033[37m"+"(매수) *성공* " + str(inputbox[0])+ "개의 비트코인을 " + str(coinmoney) + "원에 매수 했습니다!"+" (-"+str(int(inputbox[0])*int(coinmoney))+")")
        playsound("/Users/seojun/Desktop/coin/1.wav")

    if inputbox[1] == "0":
        pass
    elif haveconin < int(inputbox[1]):
        print("\033[1m"+"\033[37m"+"(매도) *실패* 보유 비트코인이 부족합니다.")
    else:
        usermoney += int(inputbox[1]) * int(coinmoney)
        haveconin -= int(inputbox[1])
        print("\033[1m"+"\033[37m"+"(매도) *성공* " + str(inputbox[1])+ "개의 비트코인을 " + str(coinmoney) + "원에 매도 했습니다!"+" (+"+str(int(inputbox[1])*int(coinmoney))+")")
        playsound("/Users/seojun/Desktop/coin/2.wav")
    
if startmoney <= usermoney:
    print("\033[1m"+"\033[37m"+"수익률 +" + str(usermoney/startmoney*100)+"%")
else:
    print("\033[1m"+"\033[37m"+"수익률 "+str((usermoney-startmoney)/startmoney*100)+"%")